...
const getFormQuery = async (id: string): Promise<IFormData> => {
      ...
      ? formFields.edges.map(
          (field: any): IFormField => ({
            ...
            choices: field.node.choices
              ? field.node?.choices.map(
                  (choice: any): IFormChoice => ({
                    text: choice.text,
                    value: choice.value,
                  })
                )
            ...
            inputs: field.node.inputs
              ? field.node?.inputs.map(
                  (input: any): IFormFieldInput => ({
                    id: input.id,
              ...
              : [],
            maxFileSize: field.node?.maxFileSize ?? null,
            allowedExtensions: field.node?.allowedExtensions ?? null,
          })
        )
      : []
